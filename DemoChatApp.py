...
class ChatClient ( Frame ):
    ...
    def handleSetServer(self):
        if self.serverSoc != None:
            ...
            self.serverStatus = 0
        serveraddr = (
        self.serverIPVar.get ().replace ( ' ', '' ), int ( self.serverPortVar.get ().replace ( ' ', '' ) ))
        try:
    ...
    def listenClients(self):
        while 1:
            ...
            threading.start_new_thread ( self.handleClientMessages, (clientsoc, clientaddr) )
        self.serverSoc.close ()
    ...
    def handleAddClient(self):
        if self.serverStatus == 0:
            ...
            return
        clientaddr = (
        self.clientIPVar.get ().replace ( ' ', '' ), int ( self.clientPortVar.get ().replace ( ' ', '' ) ))
        try:
